...
class CandidateProfileController extends Controller
{
    ...
    public function update(Request $request, string $tab): RedirectResponse
    {
        ...
        foreach (['is_public','email_allowed','sms_allowed','whatsapp_allowed','job_alerts_allowed','willing_to_relocate','willing_to_travel'] as $field) if (array_key_exists($field, $rules)) $data[$field]=$request->boolean($field);
        if ($tab === 'personal') $request->user()->update(['name'=>trim($data['first_name'].' '.$data['last_name'])]);
        if ($tab === 'preferences') { $profile->employmentTypes()->sync($data['employment_types'] ?? []); $profile->workModes()->sync($data['work_modes'] ?? []); unset($data['employment_types'],$data['work_modes']); }
        $profile->update($data);
    ...
    public function education(Request $request): RedirectResponse
    {
        ...
        $data['currently_studying']=$request->boolean('currently_studying'); $request->user()->candidateProfile->educations()->create($data);
    ...
    public function experience(Request $request): RedirectResponse
    {
        ...
        $data['currently_working']=$request->boolean('currently_working'); if ($data['currently_working']) $data['ended_on']=null; $request->user()->candidateProfile->experiences()->create($data);
    ...
    public function skill(Request $request): RedirectResponse
    {
        ...
        $request->user()->candidateProfile->skills()->syncWithoutDetaching([$data['skill_id']=>['proficiency_level_id'=>$data['proficiency_level_id'] ?? null,'years_experience'=>$data['years_experience'] ?? null,'is_primary'=>$request->boolean('is_primary')]]); return back()->with('status','Skill added.');
    }
    ...
    public function language(Request $request): RedirectResponse
    {
        ...
        $request->user()->candidateProfile->languages()->syncWithoutDetaching([$data['language_id']=>['proficiency_level_id'=>$data['proficiency_level_id'] ?? null,'is_native'=>$request->boolean('is_native')]]); return back()->with('status','Language added.');
    }

    public function remove(Request $request, string $collection, int $record): RedirectResponse
    {
        $profile=$request->user()->candidateProfile; match($collection) {'education'=>$profile->educations()->whereKey($record)->delete(),'experience'=>$profile->experiences()->whereKey($record)->delete(),'skill'=>$profile->skills()->detach($record),'language'=>$profile->languages()->detach($record),default=>abort(404)}; return back()->with('status','Entry removed.');
    }

    private function masters(): array { return ['countries'=>Country::available()->get(),'genders'=>Gender::available()->get(),'maritalStatuses'=>MaritalStatus::available()->get(),'qualificationLevels'=>QualificationLevel::available()->get(),'studyModes'=>StudyMode::available()->get(),'employmentTypes'=>EmploymentType::available()->get(),'workModes'=>WorkMode::available()->get(),'skills'=>Skill::available()->get(),'languages'=>Language::available()->get(),'proficiencyLevels'=>ProficiencyLevel::available()->get()]; }
}
