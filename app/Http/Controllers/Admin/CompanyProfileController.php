...
{
    public function edit(): View { return view('admin.company.edit',['company'=>CompanyProfile::current()]); }
    public function update(Request $request): RedirectResponse
    {
        $data=$request->validate([
            ...
            'address_line_1'=>['nullable','string','max:180'],'address_line_2'=>['nullable','string','max:180'],'city'=>['nullable','string','max:100'],'state'=>['nullable','string','max:100'],'postal_code'=>['nullable','string','max:30'],'country'=>['nullable','string','max:100'],
        ...
        $company=CompanyProfile::current(); unset($data['logo'],$data['logo_dark'],$data['favicon'],$data['cover']);
        foreach(['logo'=>'logo_path','logo_dark'=>'logo_dark_path','favicon'=>'favicon_path','cover'=>'cover_path'] as $input=>$column) if($request->hasFile($input)){if($company->{$column})Storage::disk('public')->delete($company->{$column});$data[$column]=$request->file($input)->store('company','public');}
