...
<form method="POST" action="{{ $editing ? route('recruiter.organizations.update',$organization) : route('recruiter.organizations.store') }}" class="border rounded-3 p-3 bg-light-subtle">@csrf @if($editing) @method('PUT') @endif
    <div class="profile-form-grid">
        ...
        <div><label class="form-label">City *</label><input class="form-control" name="city" required value="{{ $editing ? $organization->city : old('city') }}"></div>
        <div><label class="form-label">Postal code *</label><input class="form-control" name="postal_code" required value="{{ $editing ? $organization->postal_code : old('postal_code') }}"></div>
        <x-recruiter-location-fields :$countries :country="$editing ? $organization->country : old('country', 'India')" :state="$editing ? $organization->state : old('state')" :district="$editing ? $organization->district : old('district')" required />
