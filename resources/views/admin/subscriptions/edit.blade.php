...
<div><label class="form-label">Billing period</label><select class="form-select" name="billing_period">@foreach(['monthly','quarterly','yearly','one_time'] as $period)<option value="{{ $period }}" @selected(old('billing_period',$plan->billing_period)===$period)>{{ Str::headline($period) }}</option>@endforeach</select></div>
