...
<div class="shared-master-grid">
    <section class="dashboard-card">
        @if(($selectedType['scoped_parent'] ?? false) && $parentOptions->isNotEmpty())
            <form method="GET" action="{{ route('admin.shared-masters.index') }}" class="mb-3">
                <input type="hidden" name="type" value="{{ $selectedKey }}">
                <label class="form-label" for="master_parent_filter">Show {{ Str::lower($selectedType['label']) }} for {{ Str::lower($selectedType['parent']['label']) }}</label>
                <div class="d-flex gap-2"><select class="form-select" id="master_parent_filter" name="parent">
                    ...
                    @endforeach
                </select><button class="btn btn-outline-primary" type="submit">Apply</button></div>
            </form>
        @endif
        ...
        @forelse($records as $record)
            <tr><form method="POST" action="{{ route('admin.shared-masters.update', [$selectedKey, $record]) }}">@csrf @method('PUT')
                ...
                <td>@if(isset($selectedType['parent']))@if($selectedType['scoped_parent'] ?? false)<input type="hidden" name="{{ $selectedType['parent']['field'] }}" value="{{ $selectedParentId }}">@else<select class="form-select form-select-sm mb-1" name="{{ $selectedType['parent']['field'] }}" required><option value="">{{ $selectedType['parent']['label'] }}</option>@foreach($parentOptions as $parent)<option value="{{ $parent->id }}" @selected($record->{$selectedType['parent']['field']}===$parent->id)>{{ $parent->display_name }}</option>@endforeach</select>@endif @endif<input class="form-control form-control-sm font-monospace mb-1" name="code" value="{{ $record->code }}" required><input class="form-control form-control-sm mb-1" name="display_name" value="{{ $record->display_name }}" required><input class="form-control form-control-sm" name="short_name" value="{{ $record->short_name }}" placeholder="Short name"></td>
    ...
    <aside class="dashboard-card">
        ...
        <form method="POST" action="{{ route('admin.shared-masters.store', $selectedKey) }}">@csrf
            @if(isset($selectedType['parent']))@if($selectedType['scoped_parent'] ?? false)<input type="hidden" name="{{ $selectedType['parent']['field'] }}" value="{{ $selectedParentId }}">@else<label class="form-label" for="parent_master">{{ $selectedType['parent']['label'] }}</label><select class="form-select mb-3" id="parent_master" name="{{ $selectedType['parent']['field'] }}" required><option value="">Select {{ Str::lower($selectedType['parent']['label']) }}</option>@foreach($parentOptions as $parent)<option value="{{ $parent->id }}" @selected(old($selectedType['parent']['field'])==$parent->id)>{{ $parent->display_name }}</option>@endforeach</select>@endif @endif
